...
def graficar_resultados(df_P, df_C, df_N, n_intervalos_dia=96):
    ...
    fig, axes = plt.subplots(len(prosumidores), 1,
                              figsize=(12, 4 * len(prosumidores)), sharex=True)
...
def calcular_ahorro_por_agente(df_P, df_C, df_N, alpha, carpeta_outputs):
    ...
    axes[0].axhline(0, color="black", linewidth=0.8, linestyle="--")
    axes[0].set_xticks(x)
    axes[0].set_xticklabels(df_res["agente"], rotation=45, ha="right", fontsize=8)
    ...
    axes[1].axhline(0, color="black", linewidth=0.8, linestyle="--")
    axes[1].set_xticks(x)
    axes[1].set_xticklabels(df_res["agente"], rotation=45, ha="right", fontsize=8)
